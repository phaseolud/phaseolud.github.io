...
The temperature in a nuclear fusion reactor can be very hot, around 150 million degrees Celsius, ten times hotter than the sun. To generate the heat and energy, we burn a fuel. In this process of burning fuel, ashes are also created. These ashes must be removed because they do not burn well. However, we need to be careful when removing the ashes, because some heat can go with the ashes. The walls of the reactor can not handle too much heat. Therefore, we need to actively control the amount of heat that reaches the wall.
...
We will now look at another example, but in this case we look directly at the cross-section, and not at the whole
reactor any more. The image below on the left shows an example cross-section. The image on the
...
The camera image on the right shows the cross-section on the left, but turned around the centre. To really understand the symmetry in the reactor, and the turning around, the following video shows how a cross-section similar to the one above transforms into a camera image.
...
To get even more insight into how the camera images are created from the cross-sections, the following
interactive example allows you to draw in the cross-section, and the camera image is automatically
...
lines in the upper half or more to the right, you can see that it is much more difficult to find the original drawing back.

<DrawPoloidalCamera client:load />

As we can see, the calculation of the camera image given the cross-section happens quickly. However, transforming the camera image back to a cross-section is difficult and pretty slow. The exact reason is outside the scope of this blogpost. However, if you are interested, the example below shows why this is the case for a much smaller problem, and introduces how it can be solved faster.
